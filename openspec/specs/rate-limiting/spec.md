...
Defines a reusable, Redis-backed request-rate throttle for the backend: a
fixed-window counter keyed by route and client IP, applied via a decorator +
guard, that rejects requests exceeding a configured limit with `429`, and
fails open if the counter store is unavailable.
