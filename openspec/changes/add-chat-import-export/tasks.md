...
- [ ] 1.1 `ChatService.exportWorkspace(userId, workspaceId)`: ownership
...
- [ ] 1.2 `GET /chat/workspaces/:id/export` → `exportWorkspace(user.userId,
      id)` in `chat.controller.ts`, alongside the existing
      `workspaces/:id/messages` route

## 2. Backend — import endpoint [you implement]

- [ ] 2.1 Create `apps/backend/src/chat/dto/import-workspace.dto.ts`:
...
- [ ] 2.2 `ChatService.importWorkspace(userId, dto)`: single nested
      ...
      workspace
- [ ] 2.3 `POST /chat/workspaces/import` → `importWorkspace(user.userId,
...
- [ ] 3.1 Swagger on both new routes (`@ApiOperation`, `@ApiOkResponse`
...
- [ ] 3.2 `chat.service.spec.ts` additions: `exportWorkspace` returns
      ...
      omitted
- [ ] 3.3 Replace `chat.controller.spec.ts`'s current CLI scaffold stub
...
- [ ] 4.1 Add to `apps/frontend/lib/stores/chat.ts`: `ExportedWorkspace`
...
- [ ] 5.1 `Sidebar.tsx`: per-row Export icon button next to
...
- [ ] 5.2 `Sidebar.tsx`: "Import chat" trigger near "Create new workspace"
...
- [ ] 5.3 Inline error state for invalid JSON / failed import request
...
- [ ] 6.1 `pnpm --filter backend test` — chat suite green
- [ ] 6.2 Export a real workspace via curl/`/docs`, confirm the JSON shape;
      re-import it via the import endpoint, confirm a new workspace + all
      messages appear with preserved `createdAt`
- [ ] 6.3 Confirm import does NOT change `GET /chat/usage`'s `used` count
      and does NOT enqueue a `chat-reply` job (check queue counts via
      `/admin/queues`, already built)
- [ ] 6.4 Frontend preview: export a workspace (file downloads), import it
